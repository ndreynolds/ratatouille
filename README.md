...
Let's build a hello world application. For our purposes, an application is a
terminal application, and not necessarily an OTP application (although your
terminal application could also be an OTP application).

Our hello world application will display "Hello World" and quit when the "q" key
is pressed. First we'll look at the entire example, then we'll go through it
line by line to see what each line does. You can also find this example
[in the repo](./examples/hello_world.exs) and run it with `mix run`.
